<template>
  <UModal v-model:open="open" title="Indicators" :ui="{ content: 'bg-[#161b22] border border-[#30363d] max-w-md' }">
    <template #body>
      <div class="space-y-5">
        ...
        <!-- Overlay section -->
        <div>
          <p class="text-xs font-semibold text-[#8b949e] uppercase tracking-wider mb-2">Overlay</p>
          <div class="space-y-1">
            <button
              v-for="ind in filteredOverlay"
              :key="ind.id"
              class="w-full flex items-center gap-3 px-3 py-2 rounded-md hover:bg-[#21262d] transition-colors text-left"
            ...
            >
              ...
              <span class="text-sm text-[#e6edf3]">{{ ind.label }}</span>
            </button>
          </div>
        </div>
        ...
        <div>
          <p class="text-xs font-semibold text-[#8b949e] uppercase tracking-wider mb-2">Separate Pane</p>
          <div class="space-y-1">
            <button
